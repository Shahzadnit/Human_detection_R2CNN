...
#             mean=0.1#torch.mean(feat[:,i,:,:])
#             std=1#torch.std(feat[:,i,:,:])
#             feat[:,i,:,:]=torch.div(torch.sub(feat[:,i,:,:],mean),std)
...
class GeneralizedRCNN(nn.Module):
    ...
    def forward(self, images, targets=None, mask=None):
        ...
        # features11=features
        # m = nn.InstanceNorm2d(self.backbone.out_channels)

        print(features.shape)
        # rand_feat=torch.randn(features.shape).cuda()
        ...
        # features1 = torch.add(features1,sal_map[:,1,:,:])
        # feat=torch.mul(features,F.softmax(sal_map)[:,1,:,:])
        # print(sal_map.shape)
