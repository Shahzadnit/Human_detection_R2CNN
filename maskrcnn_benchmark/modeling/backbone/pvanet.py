...
class mCReLU_base(nn.Module):
    def __init__(self, n_in, n_out, kernelsize, stride=1, preAct=False, lastAct=True):
        ...
        # Trainable params
        self.conv3x3 = nn.Conv2d(n_in, n_out, kernelsize, stride=stride, padding=int(kernelsize/2))  # Chnaged padding to int by Sambasivarao on 12th aug 2019
...
class mCReLU_residual(nn.Module):
    def __init__(self, n_in, n_red, n_3x3, n_out, kernelsize=3, in_stride=1, proj=False, preAct=False, lastAct=True):
        ...
        self.conv3x3 = nn.Conv2d(n_red, n_3x3, kernelsize, padding=int(kernelsize/2))  #Changed padding to int by Sambasivarao K on 12th aug 2019
...
class PVANetFeat(nn.Module):
    ...
    def forward(self, x):
        ...
        # downsample1 = self.downsample1(x2)   # this should be commented
        # upsample1 = F.interpolate(x4, scale_factor=2, mode="nearest")
        upsample1 = F.interpolate(x4, scale_factor=4, mode="nearest")    ## Modifed by Shahzad 2-dec-19 This is for R2CNN
        upsample2 = F.interpolate(x3, scale_factor=2, mode="nearest")
        #x3 = torch.cat((x3, upsample1), 1)
        #upsample2 = F.interpolate(x3, scale_factor=2, mode="nearest")
        #print(x2.shape, upsample1.shape, upsample2.shape)
        #print(downsample1.shape, x3.shape, upsample1.shape)
        # features = torch.cat((downsample1, x3, upsample1), 1)
        features = torch.cat((x2, upsample2, upsample1), 1)  ## Modifed by Shahzad 2-dec-19
